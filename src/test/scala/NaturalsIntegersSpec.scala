...
with Universe with Naturals with NaturalsWithInts with Integers with IntegersWithLongs {
  ...
  describe("..") {
    it("...") {
      ...
      implicitly[NAT.zero]
      implicitly[Morphism[NAT.zero, NAT.sort]]

      implicitly[NAT.succ]
      implicitly[Morphism[NAT.succ, NAT.sort ->: NAT.sort]]
      implicitly[Morphism[NAT.succ ∙ NAT.zero, NAT.sort]]
      implicitly[Morphism[(NAT.succ ∙ (NAT.succ ∙ NAT.zero)), NAT.sort]]


      implicitly[NAT.add]
      implicitly[Morphism[NAT.add, NAT.sort ->: NAT.sort ->: NAT.sort]]
      implicitly[Morphism[NAT.add ∙ (NAT.succ ∙ NAT.zero), NAT.sort ->: NAT.sort]]
      val z = implicitly[Morphism[(NAT.add ∙ (NAT.succ ∙ NAT.zero)) ∙ NAT.zero, NAT.sort]]
      z().state shouldEqual 1
      val y = implicitly[Morphism[NAT.succ ∙ ((NAT.add ∙ (NAT.succ ∙ NAT.zero)) ∙ NAT.zero), NAT.sort]]
      y().state shouldEqual 2
      val x = implicitly[Morphism[(NAT.add ∙ (NAT.succ ∙ NAT.zero)) ∙ (NAT.succ ∙ ((NAT.add ∙ (NAT.succ ∙ NAT.zero)) ∙ NAT.zero)), NAT.sort]]
